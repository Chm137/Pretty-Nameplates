...
local function CreatePreviewFrame(parent)
    ...
    for i=1, 3 do
        ...
        a.icon:SetAllPoints()
        a.icon:SetTexture(i==1 and "Interface\\Icons\\Spell_Fire_FlameShock" or "Interface\\Icons\\Spell_Shadow_CurseOfTounges")
    ...
    ns.UpdatePreview = function()
        ...
        for i=1, 3 do
            ...
            if db.showAuras then
                ...
                a:ClearAllPoints()
                if i == 1 then a:SetPoint("BOTTOMLEFT", hp, "TOPLEFT", 0, bo)
                else a:SetPoint("LEFT", auras[i-1], "RIGHT", 1, 0) end
            else
                a:Hide()
            end
        end
        ...
        for i=1, 5 do
            ...
            if i == 1 then cps[i]:SetPoint("TOPLEFT", hp, "BOTTOMLEFT", cpx, cpOffset + cpy)
            else cps[i]:SetPoint("LEFT", cps[i-1], "RIGHT", 2, 0) end
            cps[i]:Show()
        end
...
function ns:ToggleGUI()
    if guiFrame then 
        if guiFrame:IsShown() then guiFrame:Hide() else guiFrame:Show() ns.UpdatePreview() end
        return
    end
    ...
    local function AddCheck(p, label, var, x, y)
        ...
        cb:SetScript("OnClick", function(self) PrettyNameplatesDB[var] = self:GetChecked() UpdateAll() ns.UpdatePreview() end)
        ...
        dd:SetScript("OnShow", function()
            ...
            for _, item in ipairs(list) do
                if (item.path or item.value) == PrettyNameplatesDB[key] then
                    UIDropDownMenu_SetText(dd, item.name) break
                end
            end
        end)
    end
    ...
    ForceText(cpT)
    cpT:SetPoint("TOPLEFT", 20, -140)
    cpT:SetText(L.CPHeader)
    AddSlide(t4, L.CPSize, "cpSize", 4, 20, 20, -170, false)
    AddSlide(t4, L.CPX, "cpX", -50, 50, 240, -170, false)
    AddSlide(t4, L.CPY, "cpY", -20, 20, 20, -210, false)
...
local function CreateLauncher()
    ...
    btn:SetScript("OnClick", function() InterfaceOptionsFrame:Hide() ns:ToggleGUI() end)
end
