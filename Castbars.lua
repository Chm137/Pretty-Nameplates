...
function ns:UpdateCastbar(myPlate, parentFrame)
    if not PrettyNameplatesDB.showCastbar then
    ...
    local spell, rank, displayName, iconPath, startTime, endTime, isTrade, castID, notInterruptible = UnitCastingInfo(
    unit)
    local channel = false
    if not spell then
    ...
    if spell then
        ...
        myPlate.castbar.startTime = startTime / 1000
        myPlate.castbar.endTime = endTime / 1000
        myPlate.castbar.channel = channel
        myPlate.castbar.duration = (endTime - startTime) / 1000
        ...
        myPlate.castbar:SetScript("OnUpdate", function(self, elapsed)
            local curr = GetTime()
            if self.channel then
                local val = self.endTime - curr
                self:SetValue(self.startTime + val)
                self.timeText:SetText(string.format("%.1f", math.max(0, val)))
            else
                self:SetValue(curr)
                self.timeText:SetText(string.format("%.1f", math.max(0, self.endTime - curr)))
            end
        end)
    else
        myPlate.castbar:Hide()
        myPlate.castbar:SetScript("OnUpdate", nil)
    end
end
