...
local Blacklist = {
    ["Totem"]="totem",["Тотем"]="totem",
    ["Ghoul"]="pet",["Вурдалак"]="pet",["Army"]="pet",["Войско"]="pet",
    ["Gargoyle"]="pet",["Горгулья"]="pet",["Bloodworm"]="pet",["Червь"]="pet",
    ["Imp"]="pet",["Бес"]="pet",["Voidwalker"]="pet",["Демон"]="pet",["Succubus"]="pet",["Суккуб"]="pet",
    ["Felhunter"]="pet",["Охотник"]="pet",["Felguard"]="pet",["Страж"]="pet",["Infernal"]="pet",["Инфернал"]="pet",
    ["Cat"]="pet",["Кошка"]="pet",["Boar"]="pet",["Вепрь"]="pet",["Wolf"]="pet",["Волк"]="pet",["Bear"]="pet",["Медведь"]="pet",
    ["Snake"]="pet",["Змея"]="pet",["Viper"]="pet",["Гадюка"]="pet",
    ["Elemental"]="pet",["Элементаль"]="pet",["Treant"]="pet",["Древень"]="pet",
    ["Minion"]="pet",["Прислужник"]="pet",["Guardian"]="pet",["Страж"]="pet",["Pet"]="pet",["Питомец"]="pet",["Construct"]="pet",["Конструкция"]="pet",
}
...
function ns:UpdatePlateStyle(myPlate)
    ...
    if myPlate.auras then
        ...
        for i, frame in ipairs(myPlate.auras) do
            ...
            if i == 1 then frame:SetPoint("BOTTOMLEFT", myPlate.hp, "TOPLEFT", 0, bottomOffset)
            else frame:SetPoint("LEFT", myPlate.auras[i-1], "RIGHT", 1, 0) end
    ...
    if myPlate.important then
        ...
        for i, frame in ipairs(myPlate.important) do
            ...
            frame:ClearAllPoints()
            if i == 1 then frame:SetPoint("LEFT", myPlate.hp, "RIGHT", 6, 0)
            else frame:SetPoint("LEFT", myPlate.important[i-1], "RIGHT", 4, 0) end
...
function ns:CreatePlateFrame(parentFrame)
    ...
    myPlate:SetScript("OnUpdate", function(self, elapsed)
        ...
        for _, region in pairs(regions) do
            if region and not region.pnp_ignore then 
                ...
                elseif type == "FontString" then
                    ...
                    if text then
                        if tonumber(text) or text == "??" then region:SetAlpha(0)
                        else
        ...
        if shouldHide then
            self.hp:Hide() self.totemIcon:Hide() self:SetScale(0.001) self:SetAlpha(0)
            return
        else
        ...
        if classColor then
            ...
            if db.tankMode then
                if hasAggro then self.hp:SetStatusBarColor(db.hpColor.r, db.hpColor.g, db.hpColor.b)
                else self.hp:SetStatusBarColor(0.6, 0.6, 0.6) end
            else
                if r > 0.8 and g > 0.8 and b < 0.3 then
                    if db.colorNeutral then self.hp:SetStatusBarColor(db.hpColor.r, db.hpColor.g, db.hpColor.b)
                    else self.hp:SetStatusBarColor(r, g, b) end
        ...
        if self.cps and UnitName("target") == name and isTarget then
            ...
            for i=1, 5 do
                ...
                else self.cps[i]:Hide() end
            end
        elseif self.cps then for i=1, 5 do self.cps[i]:Hide() end end
